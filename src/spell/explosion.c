...
void explosion_init(struct explosion* explosion, struct spell_data_source* source, struct spell_event_options event_options) {
    explosion->render_id = render_scene_add(&r_scene_3d, &explosion->data_source->position, 4.0f, (render_scene_callback)explosion_render, explosion);
...
void explosion_destroy(struct explosion* explosion) {
    render_scene_remove(&r_scene_3d, explosion->render_id);
