...
filesystem/meshes/%.tmesh filesystem/meshes/%.anim: assets/meshes/%.blend tools/mesh_export/t3d_mesh.d.template
	...
	$(BLENDER_5) $< --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/t3d_mesh.py -- $(@:filesystem/meshes/%.tmesh=build/assets/meshes/%.tmesh)
...
assets/game_objects.json: tools/mesh_export/rebuild_prefabs.py $(EXPORT_SOURCE)
	$(BLENDER_5) --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/rebuild_prefabs.py -- assets/game_objects.json
...
filesystem/%.mat: assets/%.mat.blend tools/mesh_export/material_fast64.d.template
	@mkdir -p $(dir $@)
	@mkdir -p $(dir $(@:filesystem/%.mat=build/assets/%.mat))
	$(BLENDER_5) $< --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/material_fast64.py -- $@
...
assets/materials/materials.blend: tools/mesh_export/material_generator.py $(MATERIAL_SOURCES)
	$(BLENDER_5) --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/material_generator.py -- $@ $(MATERIAL_SOURCES)
...
build/assets/scripts/globals.json build/assets/scripts/globals.dat src/player/inventory_mapping.c src/cutscene/global_list.h: tools/mesh_export/globals.py assets/scripts/globals.script build/cutscene/function_defs.json
...
filesystem/scenes/%.scene: assets/scenes/%.blend $$(wildcard assets/scenes/%.script) build/assets/scripts/globals.json build/cutscene/function_defs.json tools/mesh_export/scene.d.template
	...
	echo $@ $<
	$(BLENDER_5) $< --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/scene.py -- $(@:filesystem/scenes/%.scene=build/assets/scenes/%.scene) $(@:%.scene=%.overworld)
...
build/assets/scenes/%_exits.txt: assets/scenes/%.blend tools/mesh_export/find_exits.py
	@mkdir -p $(dir $@)
	$(BLENDER_5) $< --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/find_exits.py -- $@
...
build/blender_results.txt: $(EXPORT_SOURCE)
	@mkdir -p $(dir $@)
	$(BLENDER_5) --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/tests.py
	echo "success" > $@
...
build/strings.txt: tools/mesh_export/export_text.py $(SCRIPTS)
	$(BLENDER_5) --background --addons fast64-main --python-exit-code 1 --python tools/mesh_export/export_text.py -- $@ $(SCRIPTS)
